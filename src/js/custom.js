...
function serverIdentify() {
        ...
        if (this.readyState === 4 && this.status === 200){
            if(this.responseText !== null && this.responseText !== ""){
                ...
                if (response !== "failed" && response !== null) {
                    Swal.fire({
                        title: "Good job, " + response.data.fullname + "!",
                        text: response.message,  // Display the message from the response
                        icon: "success"
                    }).then(() => {
                        window.location.href = "http://localhost/fingerprint/view/dashboard.php"; // Change this to your desired page URL
                    });
    ...
    xhttp.open("POST", "/FingerPrint/src/core/verify.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send(payload);
}
